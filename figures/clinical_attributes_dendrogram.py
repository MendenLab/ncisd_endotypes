...
def perform_test_continuous_variables(df, continuous_cols, encoded__labels, save_folder):
    ...
    names = unique_labels.tolist()

    dict_all_continuous_attributes = {
        "Anova": pd.DataFrame(index=list(df_continuous.columns),
                              columns=["{} vs {}".format(names[0], names[1])]),
        "Welch's t-test": pd.DataFrame(index=list(df_continuous.columns),
                                       columns=["{} vs {}".format(names[0], names[1])]),
        "Kruskal-Wallis": pd.DataFrame(index=list(df_continuous.columns),
                                       columns=["{} vs {}".format(names[0], names[1])])}
    ...
    df_temp['group'] = encoded__labels.tolist()
    ...
    for ind_var_normal in df_var_normal_test.index:
        ...
        if df_var_normal_test.loc[ind_var_normal, 'Normal_distribution']:
            ...
            if df_var_normal_test.loc[ind_var_normal, 'var_homogeneity_pval'] > 0.05:
                ...
                _, p_temp = f_oneway(variable_classes[0], variable_classes[1], axis=0)
    ...
    df_anova = pd.concat([
        df_anova, pd.DataFrame(d_swap, index=["{} vs {}".format(names[0], names[1])]).T], axis=1)
    d_swap = {v: k for k, v in dict(pval_list["Welch's t-test"]).items()}
    df_welch = pd.concat([
        df_welch, pd.DataFrame(d_swap, index=["{} vs {}".format(names[0], names[1])]).T], axis=1)
    d_swap = {v: k for k, v in dict(pval_list['Kruskal-Wallis']).items()}
    df_kruskal = pd.concat([
        df_kruskal, pd.DataFrame(d_swap, index=["{} vs {}".format(names[0], names[1])]).T], axis=1)

    # Save effect size
    df_effect_size = pd.concat([
        df_effect_size,
        pd.DataFrame(dict(effect_size), index=["{} vs {}".format(names[0], names[1])]).T], axis=1)
    df_effect_size_categorised = pd.concat([
        df_effect_size_categorised,
        pd.DataFrame(dict(effect_size_categorised), index=["{} vs {}".format(names[0], names[1])]).T], axis=1)

    df_mean_group_0 = pd.concat([
        df_mean_group_0, pd.DataFrame(dict(mean_group_0), index=[names[0]]).T], axis=1)
    df_mean_group_rest = pd.concat([
        df_mean_group_rest, pd.DataFrame(
            dict(mean_group_rest), index=["{} ({})".format(names[1], names[0])]).T], axis=1)
    ...
    for test_key in pval_list.keys():
        if len(pval_list[test_key]) > 0:
            for df_val, attr in pval_list[test_key]:
                ...
                dict_all_continuous_attributes[test_key].loc[attr, "{} vs {}".format(names[0], names[1])] = df_val

    return (dict_all_continuous_attributes, df_effect_size, df_anova, df_welch, df_kruskal, df_effect_size_categorised,
            df_mean_group_0, df_mean_group_rest)
...
def perform_test_ordinal_category(df_scaled, ordinal_cols, encoded__labels, save_folder):
    ...
    df_all_ordinal_attributes = pd.DataFrame(
        index=list(df_ordinal.columns), columns=["{} vs {}".format(names[0], names[1])])
    df_all_ordinal_attributes_effect_size = pd.DataFrame(
        index=list(df_ordinal.columns), columns=["{} vs {}".format(names[0], names[1])])
    df_kruskal = pd.DataFrame()
    df_effect_size = pd.DataFrame()
    df_effect_size_categorised = pd.DataFrame()
    ...
    df_median_group_0 = pd.concat([
        df_median_group_0, pd.DataFrame(dict(median_group_0), index=[names[0]]).T], axis=1)
    df_median_group_1 = pd.concat([
        df_median_group_1, pd.DataFrame(dict(median_group_1), index=["{} ({})".format(names[1], names[0])]).T], axis=1)
    df_mean_group_0 = pd.concat([
        df_mean_group_0, pd.DataFrame(dict(mean_group_0), index=[names[0]]).T], axis=1)
    df_mean_group_1 = pd.concat([
        df_mean_group_1, pd.DataFrame(dict(mean_group_1), index=["{} ({})".format(names[1], names[0])]).T], axis=1)
    ...
    if len(pval_list[test_key]) > 0:
        for df_pval, attr in pval_list[test_key]:
            ...
            df_all_ordinal_attributes.loc[attr, "{} vs {}".format(names[0], names[1])] = df_pval

    if len(pval_list[test_key]) > 0:
        for attr, effect in effect_size:
            # Save padj values for each pairwise comparision in dataframe
            df_all_ordinal_attributes_effect_size.loc[attr, "{} vs {}".format(names[0], names[1])] = effect

    return (df_all_ordinal_attributes, df_all_ordinal_attributes_effect_size, df_effect_size_categorised,
            df_median_group_0, df_median_group_1, df_mean_group_0, df_mean_group_1)
...
def lookup_oddsratio(odds_ratio):
    ...
    elif (odds_ratio < 0.92) & (odds_ratio >= 0.71):
...
def perform_test_nominal_category(df, nominal_cols, encoded__labels, save_folder):
    ...
    df_all_nominal_attributes = pd.DataFrame(
        index=list(df_nominal.columns), columns=["{} vs {}".format(names[0], names[1])])
    df_all_nominal_attributes_effect_size = pd.DataFrame(
        index=list(df_nominal.columns), columns=["{} vs {}".format(names[0], names[1])])
    ...
    for nominal_attr in nominal_cols_encoded:
        ...
        if (len(df_melted_tmp['value'].unique()) > 1) & (len(df_melted_tmp['variable'].unique()) > 1):
    ...
    df_chisquare = pd.concat([
        df_chisquare, pd.DataFrame(d_swap, index=["{} vs {}".format(names[0], names[1])]).T], axis=1)
    ...
    df_mode_group_0 = pd.concat([
        df_mode_group_0, pd.DataFrame(dict(mode_group_0), index=[names[0]]).T], axis=1)
    df_mode_group_rest = pd.concat([
        df_mode_group_rest, pd.DataFrame(dict(mode_group_rest), index=["{} ({})".format(names[1], names[0])]).T], axis=1)
    ...
    if len(pval_list[test_key]) > 0:
        ...
        for pvals, attr in pval_list[test_key]:
            # Save padj values for each pairwise comparison in dataframe
            df_all_nominal_attributes.loc[attr, "{} vs {}".format(names[0], names[1])] = pvals

    if len(pval_list[test_key]) > 0:
        for attr, effect in effect_size:
            # Save padj values for each pairwise comparision in dataframe
            df_all_nominal_attributes_effect_size.loc[attr, "{} vs {}".format(names[0], names[1])] = effect

    return (df_all_nominal_attributes, df_all_nominal_attributes_effect_size, df_effect_size_categorised,
            df_mode_group_0, df_mode_group_rest)


def calculate_padj_values(df_merged):
...
def categorise_effect_size(df_effect_size, apply_func):
    # Categorise effect size (Cohen's D) of continuous attributes
    df_effect_size_categorised = pd.DataFrame(
        index=df_effect_size.index, columns=df_effect_size.columns)
    for idx in df_effect_size_categorised.index:
        for col in df_effect_size_categorised.columns:
            df_effect_size_categorised.loc[idx][col] = apply_func(df_effect_size.loc[idx][col])

    return df_effect_size_categorised
...
def plot_summary(df, save_folder, key, figsize=(16, 6)):
    ...
    if df_tmp_.shape[0] > 0:
        ...
        cbar.ax.set_ylabel('-log10(p-adj values)', rotation=90, fontsize=12)
        plt.tight_layout()
...
def plot_comparisons_summary(df, df_effect_size, save_folder, key, xlabels, pval_cut=0.05, figsize=(16, 12)):
    df = df.dropna(axis='index', how='all').astype(float)
    df_effect_size = df_effect_size.dropna(axis='index', how='all').astype(float)
    df_copy = df.copy()
    ...
    df = df.loc[index]
    df_tmp_ = -np.log10(df)
    # Add sign of effect size
    df_tmp_[df_effect_size.loc[index] < 0] = df_tmp_[df_effect_size.loc[index] < 0] * -1
    cmap_reds = plt.get_cmap('Reds')
    cmap_blues = plt.get_cmap('Blues')
    num_colors = 80  # int(np.ceil(np.amax(df_tmp_) / (-np.log10(0.05))))
    if pval_cut == 0.05:
        colors = [cmap_blues(i / num_colors) for i in range(4, num_colors + 4)][::-1] + [
            'lightgrey'] * (num_colors - 68) + [cmap_reds(i / num_colors) for i in range(4, num_colors + 4)]
    else:
        colors = [cmap_blues(i / num_colors) for i in range(4, num_colors + 4)][::-1] + [
            'lightgrey'] * (num_colors - 21) + [cmap_reds(i / num_colors) for i in range(4, num_colors + 4)]
    cmap = LinearSegmentedColormap.from_list('', colors, len(colors))
    ...
    df_colors = pd.DataFrame(
        data=np.asarray([["#ff6db6"], ["#490092"],
                         ["#b66dff"], ["#000000"], ["#920000"], ["#E69F00"], ["#D55E00"], ["#8B4513"],
                         ["#999999"], ["#006ddb"], ["#b6dbff"], ["#004949"]]).T, columns=df.columns)
    ...
    # Pattern 2a-like
    df_pattern_2a = df_tmp_[['E11, E12, E13, E1, E2, E3, E4 vs E5, E6, E7, E8, E9, E10',
                             'E5 vs E6', 'E10 vs E9', 'E8 vs E9, E10', 'E7 vs E8, E9, E10',
                             'E5, E6 vs E7, E8, E9, E10']]
    axmatrix = fig.add_axes([0.2, 0.1, 0.28, 0.72])  # (left, bottom, width, height)
    sns.heatmap(df_pattern_2a,
                xticklabels=True, yticklabels=True, ax=axmatrix,
                vmin=np.amin(df_tmp_), vmax=np.amax(df_tmp_), center=0,
                mask=(df_pattern_2a < -np.log10(pval_cut)) & (df_pattern_2a > np.log10(pval_cut)),
                cmap=cmap, cbar=False, linewidths=1.3)
    ...
    axmatrix.set_yticks([])
    axmatrix.set_xticklabels(['E11, E12, E13, E1, E2, E3, E4 vs E5, E6, E7, E8, E9, E10',
                              'E5 vs E6', 'E10 vs E9', 'E8 vs E9, E10', 'E7 vs E8, E9, E10',
                              'E5, E6 vs E7, E8, E9, E10'], rotation=90)
    ...
    # Pattern 3-like
    df_pattern_3 = df_tmp_[['E11 vs E12, E13', 'E12 vs E13']]
    axmatrix = fig.add_axes([0.5, 0.1, 0.14, 0.72])  # (left, bottom, width, height)
    sns.heatmap(df_pattern_3, xticklabels=True, yticklabels=True, ax=axmatrix,
                vmin=np.amin(df_tmp_), vmax=np.amax(df_tmp_), center=0,
                mask=(df_pattern_3 < -np.log10(pval_cut)) & (df_pattern_3 > np.log10(pval_cut)),
                cmap=cmap, cbar=False, linewidths=1.3)
    axmatrix.set_yticklabels(axmatrix.get_yticklabels(), rotation=0)
    axmatrix.set_xticklabels(axmatrix.get_xticklabels(), rotation=90)
    axmatrix.set_yticklabels([])
    axmatrix.set_yticks([])
    axmatrix.set_xticklabels(['E11 vs E12, E13', 'E12 vs E13'], rotation=90)
    axmatrix.spines['top'].set_visible(True)
    axmatrix.spines['right'].set_visible(True)
    axmatrix.spines['bottom'].set_visible(True)
    axmatrix.spines['left'].set_visible(True)
    ...
    # Pattern 1-like
    df_pattern_1 = df_tmp_[['E1, E2 vs E3, E4', 'E1 vs E2',  'E3 vs E4']]
    axmatrix = fig.add_axes([0.66, 0.1, 0.19, 0.72])  # (left, bottom, width, height)
    sns.heatmap(df_pattern_1, xticklabels=True, yticklabels=True, ax=axmatrix,
                vmin=np.amin(df_tmp_), vmax=np.amax(df_tmp_), center=0,
                mask=(df_pattern_1 < -np.log10(pval_cut)) & (df_pattern_1 > np.log10(pval_cut)),
                cmap=cmap, cbar=False, linewidths=1.3)
    axmatrix.set_yticklabels(axmatrix.get_yticklabels(), rotation=0)
    axmatrix.set_xticklabels(axmatrix.get_xticklabels(), rotation=90)
    axmatrix.yaxis.set_label_position("right")
    axmatrix.yaxis.tick_right()
    axmatrix.set_xticklabels(['E1, E2 vs E3, E4', 'E1 vs E2',  'E3 vs E4'], rotation=90)
    axmatrix.spines['top'].set_visible(True)
    axmatrix.spines['right'].set_visible(True)
    axmatrix.spines['bottom'].set_visible(True)
    axmatrix.spines['left'].set_visible(True)
    ...
    # Add colorbar
    axcolor = fig.add_axes([0.2, -0.3, 0.65, 0.02])  # (left, bottom, width, height) fig.add_axes([0.9, 0.1, 0.03, 0.72])
    if pval_cut == 0.05:
        cbar = plt.colorbar(
            mappable=axmatrix.collections[0], cax=axcolor, orientation="horizontal",
            ticks=[np.amin(df_tmp_)] + [-3] + [0] + [4, 8, 12] + [np.amax(df_tmp_)])
    else:
        cbar = plt.colorbar(
            mappable=axmatrix.collections[0], cax=axcolor, orientation="horizontal",
            ticks=[np.amin(df_tmp_)] + [-12] + [
                np.log10(pval_cut)] + [0] + [-np.log10(pval_cut)] + [12] + [np.amax(df_tmp_)])
    # cbar.ax.get_xaxis().labelpad = 15
    cbar.ax.set_xlabel('signed[-log10(padj. value)]', rotation=0, fontsize=12)

    # # Add heatmap showing the colors of the clusters
    # ax_colors = fig.add_axes([0.2, 0.825, 0.65, 0.02])  # (left, bottom, width, height)
...
def main(input_dir, save_folder):
    ...
    # might have to adjust some variables -> Peter
    ...
    df, nominal_cols, ordinal_cols, continuous_cols = prepare_attributes(adata=adata, save_folder=save_folder)

    obs_name = 'Endotypes'  # 'Molecular Subtype res0.9'

    # Load dendrogram comparisons
    pairs = pd.read_excel(os.path.join(general_dir, 'analysis', 'Molecular_subtypes', 'output',
                                       'Pathways/Endotypes_Dendrogram/2023-09-13/Dendrogram_pairs.xlsx'))

    df_dendorgram_pvalue = pd.DataFrame()
    df_dendorgram_padj = pd.DataFrame()
    df_dendorgram_effectsize = pd.DataFrame()
    df_dendorgram_effectsize_categorised = pd.DataFrame()

    df_continuous_all_anova = pd.DataFrame()
    df_continuous_all_welch = pd.DataFrame()
    df_continuous_all_kruskal = pd.DataFrame()
    df_continuous_all_effect_size = pd.DataFrame()
    df_continuous_all_effect_size_categorised = pd.DataFrame()
    df_continuous_all_mean_group_0 = pd.DataFrame()
    df_continuous_all_mean_group_rest = pd.DataFrame()

    df_ordinal_all_pval = pd.DataFrame()
    df_ordinal_all_effect_size = pd.DataFrame()
    df_ordinal_all_effect_size_categorised = pd.DataFrame()
    df_ordinal_all_mean_group_0 = pd.DataFrame()
    df_ordinal_all_mean_group_rest = pd.DataFrame()
    df_ordinal_all_median_group_0 = pd.DataFrame()
    df_ordinal_all_median_group_rest = pd.DataFrame()

    df_nominal_all_pval = pd.DataFrame()
    df_nominal_all_effect_size = pd.DataFrame()
    df_nominal_all_effect_size_categorised = pd.DataFrame()
    df_nominal_all_mode_group_0 = pd.DataFrame()
    df_nominal_all_mode_group_rest = pd.DataFrame()

    # 1 vs 1
    for pair_ind in range(0, pairs.shape[0]):
        obs_name_tmp = 'Endotypes_tmp'
        # Read out comparisons
        pair_0 = ['E' + str(-1 * int(s)) for s in pairs.loc[pair_ind, 'V1'].split(', ')]
        pair_1 = ['E' + str(-1 * int(s)) for s in pairs.loc[pair_ind, 'V2'].split(', ')]

        pair_name = pair_0 + pair_1
        # Subset data to Endotypes of interest
        adata_tmp = adata[adata.obs['Endotypes'].isin(pair_name)].copy()
        df_tmp = df.loc[adata_tmp.obs.index, :].copy()

        adata_tmp.obs[obs_name_tmp] = 'E0'
        adata_tmp.obs.loc[adata_tmp.obs['Endotypes'].isin(pair_0), obs_name_tmp] = ', '.join(pair_0)
        adata_tmp.obs.loc[adata_tmp.obs['Endotypes'].isin(pair_1), obs_name_tmp] = ', '.join(pair_1)
        adata_tmp.obs[obs_name_tmp] = adata_tmp.obs[obs_name_tmp].astype('category')

        # Encode labels
        encoded__labels = adata_tmp.obs[obs_name_tmp].values

        # 1. Continuous variables
        (dict_all_continuous_attributes, df_all_continuous_attributes_effect_size, df_anova, df_welch,
         df_kruskal, df_continuous_effect_size_categorised,
         df_continuous_mean_group_0, df_continuous_mean_group_rest) = perform_test_continuous_variables(
            df=df_tmp, continuous_cols=continuous_cols, encoded__labels=encoded__labels, save_folder=save_folder)

        # 2. Ordinal (Dichotomus) categories
        (df_all_ordinal_attributes, df_all_ordinal_attributes_effect_size, df_ordinal_effect_size_categorised,
            df_ordinal_median_group_0, df_ordinal_median_group_1,
         df_ordinal_mean_group_0, df_ordinal_mean_group_1) = perform_test_ordinal_category(
            df_scaled=df_tmp, ordinal_cols=ordinal_cols, encoded__labels=encoded__labels, save_folder=save_folder)

        # 3. Nominal categories
        (df_all_nominal_attributes, df_all_nominal_attributes_effect_size, df_nominal_effect_size_categorised,
         df_mode_group_0, df_mode_group_rest) = perform_test_nominal_category(
            df=df_tmp, nominal_cols=nominal_cols, encoded__labels=encoded__labels, save_folder=save_folder)

        # -------- p-values
        df_all_continuous_attributes = pd.DataFrame()
        for key_name in dict_all_continuous_attributes.keys():
        ...
        # Calculate padj values
        df_padj = calculate_padj_values(df_merged)

        # -------- Effect size
        # Drop na rows in continuous effect size (Cohen's d)
        ...
        # Categorise effect size (Cohen's D) of continuous attributes
        df_all_continuous_attributes_effect_size_categorised = categorise_effect_size(
            df_effect_size=df_all_continuous_attributes_effect_size, apply_func=categorize_cohens_d)
        # Categorise effect size (Cliff's delta) of ordinal attributes
        df_all_ordinal_attributes_effect_size_categorised = categorise_effect_size(
            df_effect_size=df_all_ordinal_attributes_effect_size, apply_func=lookup_size)
        # Categorise effect size (Odds Ratio) of nominal attributes
        df_all_nominal_attributes_effect_size_categorised = categorise_effect_size(
            df_effect_size=df_all_nominal_attributes_effect_size, apply_func=lookup_oddsratio)
        ...
        comparison = df_combined['Comparison'].unique()[0]
        df_combined['Effect'] = df_combined['Effect'].astype(str)

        df_combined.loc[:, 'color'] = df_combined.loc[:, 'Effect'].map(effect_color_map)

        # Sort by signed padj values for the waterfall effect
        df_combined = df_combined.sort_values(by='signed_padj').reset_index(drop=True)

        # Drop all attributes with padj value > 0.2
        df_combined = df_combined[df_combined['padj'] < 0.2]

        # Filter the effect_colors dictionary to include only the effects present in the data
        used_effect_colors = {
            effect: color for effect, color in effect_color_map.items() if effect in df_combined['Effect'].unique()}

        # Sort the dataframe by (Effect), then by signed_padj (negative values in descending order)
        df_combined = df_combined.sort_values(by=['signed_padj'], ascending=False).reset_index(drop=True)

        fig, ax = plt.subplots(figsize=(8, 4))
        # Plot bars
        ax.bar(df_combined.index, df_combined['signed_padj'], color=df_combined['color'])
        ...
        ax.set_xticklabels(df_combined['index'], rotation=90, fontsize=8)
        # Adding labels
        ...
        df_dendorgram_pvalue = pd.concat([df_dendorgram_pvalue, df_merged], axis=1)
        df_dendorgram_padj = pd.concat([df_dendorgram_padj, df_padj], axis=1)
        df_dendorgram_effectsize = pd.concat([df_dendorgram_effectsize, df_merged_effect_size], axis=1)
        df_dendorgram_effectsize_categorised = pd.concat([
            df_dendorgram_effectsize_categorised, df_effectsize_categorised], axis=1)

        # Continuous
        df_continuous_all_anova = pd.concat([df_continuous_all_anova, df_anova], axis=1)
        df_continuous_all_welch = pd.concat([df_continuous_all_welch, df_welch], axis=1)
        df_continuous_all_kruskal = pd.concat([df_continuous_all_kruskal, df_kruskal], axis=1)
        df_continuous_all_effect_size = pd.concat([
            df_continuous_all_effect_size, df_all_continuous_attributes_effect_size], axis=1)
        df_continuous_all_effect_size_categorised = pd.concat([
            df_continuous_all_effect_size_categorised, df_continuous_effect_size_categorised], axis=1)
        df_continuous_all_mean_group_0 = pd.concat([
            df_continuous_all_mean_group_0, df_continuous_mean_group_0], axis=1)
        df_continuous_all_mean_group_rest = pd.concat([
            df_continuous_all_mean_group_rest, df_continuous_mean_group_rest], axis=1)

        # Ordinal
        df_ordinal_all_pval = pd.concat([df_ordinal_all_pval, df_all_ordinal_attributes], axis=1)
        df_ordinal_all_effect_size = pd.concat([
            df_ordinal_all_effect_size, df_all_ordinal_attributes_effect_size], axis=1)
        df_ordinal_all_effect_size_categorised = pd.concat([
            df_ordinal_all_effect_size_categorised, df_ordinal_effect_size_categorised], axis=1)
        df_ordinal_all_mean_group_0 = pd.concat([df_ordinal_all_mean_group_0, df_ordinal_mean_group_0], axis=1)
        df_ordinal_all_mean_group_rest = pd.concat([df_ordinal_all_mean_group_rest, df_ordinal_mean_group_1], axis=1)
        df_ordinal_all_median_group_0 = pd.concat([df_ordinal_all_median_group_0, df_ordinal_median_group_0], axis=1)
        df_ordinal_all_median_group_rest = pd.concat([
            df_ordinal_all_median_group_rest, df_ordinal_median_group_1], axis=1)

        # Nominal
        df_nominal_all_pval = pd.concat([df_nominal_all_pval, df_all_nominal_attributes], axis=1)
        df_nominal_all_effect_size = pd.concat([
            df_nominal_all_effect_size, df_all_nominal_attributes_effect_size], axis=1)
        df_nominal_all_effect_size_categorised = pd.concat([
            df_nominal_all_effect_size_categorised, df_nominal_effect_size_categorised], axis=1)
        df_nominal_all_mode_group_0 = pd.concat([df_nominal_all_mode_group_0, df_mode_group_0], axis=1)
        df_nominal_all_mode_group_rest = pd.concat([df_nominal_all_mode_group_rest, df_mode_group_rest], axis=1)

    # Save stats continous to excel sheet
    df_continuous_all_anova.to_excel(os.path.join(save_folder, 'Stats_Continuous.xlsx'), sheet_name='Anova')
    # Save padj values to excel file
    with pd.ExcelWriter(os.path.join(save_folder, 'Stats_Continuous.xlsx'),
                        mode="a", engine="openpyxl") as writer:
        df_continuous_all_welch.to_excel(writer, sheet_name="Welch's t-test", index=True)
        df_continuous_all_kruskal.to_excel(writer, sheet_name='Kruskal-Wallis', index=True)
        df_continuous_all_effect_size.to_excel(writer, sheet_name="Cohen's D", index=True)
        df_continuous_all_effect_size_categorised.to_excel(writer, sheet_name="Cohen's D categorised", index=True)
        df_continuous_all_mean_group_0.to_excel(writer, sheet_name='Mean group Group 1', index=True)
        df_continuous_all_mean_group_rest.to_excel(writer, sheet_name='Mean group Group 2', index=True)

    # Save Stats of ordinal to excel sheet
    df_ordinal_all_pval.to_excel(os.path.join(save_folder, 'Stats_Ordinal.xlsx'), sheet_name='Kruskal-Wallis')
    with pd.ExcelWriter(os.path.join(save_folder, 'Stats_Ordinal.xlsx'),
                        mode="a", engine="openpyxl") as writer:
        df_ordinal_all_effect_size.to_excel(writer, sheet_name="Cliff's Delta", index=True)
        df_ordinal_all_effect_size_categorised.to_excel(writer, sheet_name="Cliff's Delta categorised", index=True)
        df_ordinal_all_mean_group_0.to_excel(writer, sheet_name='Median group Endotypes', index=True)
        df_ordinal_all_mean_group_rest.to_excel(writer, sheet_name='Median group Rest', index=True)
        df_ordinal_all_median_group_0.to_excel(writer, sheet_name='Mean group Group 1', index=True)
        df_ordinal_all_median_group_rest.to_excel(writer, sheet_name='Mean group Group 2', index=True)

    # Save Stats of nominal to excel sheet
    df_nominal_all_pval.to_excel(os.path.join(save_folder, 'Stats_Nominal.xlsx'), sheet_name='Chi-square')
    with pd.ExcelWriter(os.path.join(save_folder, 'Stats_Nominal.xlsx'),
                        mode="a", engine="openpyxl") as writer:
        df_nominal_all_effect_size.to_excel(writer, sheet_name="Cliff's Delta", index=True)
        df_nominal_all_effect_size_categorised.to_excel(writer, sheet_name="Cliff's Delta categorised", index=True)
        df_nominal_all_mode_group_0.to_excel(writer, sheet_name='Mode group Endotypes', index=True)
        df_nominal_all_mode_group_rest.to_excel(writer, sheet_name='Mode group Rest', index=True)

    # Save all in one excel sheet
    df_dendorgram_pvalue.to_excel(
        os.path.join(save_folder, 'Stats_enriched_clinical_attributes_Dendrogram.xlsx'),
        sheet_name='p-value', index=True)
    with pd.ExcelWriter(
            os.path.join(save_folder, 'Stats_enriched_clinical_attributes_Dendrogram.xlsx'),
            mode="a", engine="openpyxl") as writer:
        df_dendorgram_padj.to_excel(writer, sheet_name='padj. value', index=True)
        df_dendorgram_effectsize.to_excel(writer, sheet_name="effect size", index=True)
        df_dendorgram_effectsize_categorised.to_excel(writer, sheet_name="effect size categorised", index=True)

    # Plot Heatmaps
    pval_cut = 0.05  # 1e-5  # 0.05
    figsize = (8, 12)  # (5, 6)  # (8, 12)
    plot_summary(df=df_dendorgram_padj, save_folder=save_folder, key='merged', figsize=(16, 8))
    # Apply unsupervised clustering on clinical attributes - which attributes do cluster together?
    # plot_clustered_summary(df=df_dendorgram, save_folder=save_folder, key='merged', figsize=(20, 18))
    # plot_clustered_comparisons_summary(df=df_dendorgram, save_folder=save_folder, key='merged', figsize=(14, 14))
    plot_comparisons_summary(
        df=df_dendorgram_padj, df_effect_size=df_dendorgram_effectsize, save_folder=save_folder,
        xlabels=list(adata.obs[obs_name].cat.categories), pval_cut=pval_cut, key='merged_thesis', figsize=figsize)
...
if __name__ == '__main__':
    ...
    output_dir = os.path.join(general_dir, 'analysis', 'Molecular_subtypes', 'output',
                              'clinical_attr_dendrogram', str(date.today()))
