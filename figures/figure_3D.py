...
from statsmodels.sandbox.stats.multicomp import multiplecomptests
...
def prepare_attributes(adata, input_dir_attr, save_folder):
    ...
    for col in cat_cols:
        ...
        if len(pvals) > 0:
            reject_list, corrected_p_vals = multiplecomptests(pvals, method='fdr_bh')[:2]
    ...
    for col in cat_cols:
        ...
        if len(pvals_cat_con) > 0:
            reject_list, corrected_p_vals = multiplecomptests(pvals_cat_con, method='fdr_bh')[:2]
...
def categorize_cohens_d(cohens_d):
    ...
    if abs(cohens_d) < 0.2:
        return dict_sign[str(np.sign(cohens_d))] + ", Very small effect"
...
def perform_test_continuous_variables(df, continuous_cols, encoded__labels, save_folder):
    ...
    for group in names:
        ...
        df_effect_size = pd.concat([
            df_effect_size, pd.DataFrame(dict(effect_size), index=[group]).T], axis=1)
        df_effect_size_categorised = pd.concat([
            df_effect_size_categorised, pd.DataFrame(dict(effect_size_categorised), index=[group]).T], axis=1)

        df_mean_group_0 = pd.concat([
            df_mean_group_0, pd.DataFrame(dict(mean_group_0), index=[group]).T], axis=1)
    ...
    with pd.ExcelWriter(os.path.join(save_folder_tmp, 'Stats.xlsx'),
        ...
        df_mean_group_0.to_excel(writer, sheet_name='Mean group 0', index=True)
...
def lookup_size(delta: float, dull: dict = None) -> str:
    ...
    if abs(delta) < dull['small']:
        return dict_sign[str(np.sign(delta))] + ', negligible'
    if dull['small'] <= abs(delta) < dull['medium']:
        return dict_sign[str(np.sign(delta))] + ', small'
    if dull['medium'] <= abs(delta) < dull['large']:
        return dict_sign[str(np.sign(delta))] + ', medium'
    if abs(delta) >= dull['large']:
        return dict_sign[str(np.sign(delta))] + ', large'
...
def perform_test_ordinal_category(df_scaled, ordinal_cols, encoded__labels, save_folder):
    ...
    for group in names:
        ...
        for ordinal_attr in ordinal_cols:
            ...
            mean_group_0.append((ordinal_attr, np.median(variable_classes[0].astype(float))))
            mean_group_1.append((ordinal_attr, np.median(variable_classes[1].astype(float))))

        # Save Kruskal-Wallis test results to dataframe
        d_swap = {v: k for k, v in dict(pval_list['Kruskal']).items()}
        df_kruskal = pd.concat([df_kruskal, pd.DataFrame(d_swap, index=[group[0]]).T], axis=1)

        # Save effect size
        df_effect_size = pd.concat(
            [df_effect_size, pd.DataFrame(dict(effect_size), index=[group[0]]).T], axis=1)
        df_effect_size_categorised = pd.concat(
            [df_effect_size_categorised, pd.DataFrame(dict(effect_size_categorised), index=[group[0]]).T], axis=1)

        df_median_group_0 = pd.concat([
            df_median_group_0, pd.DataFrame(dict(mean_group_0), index=[group[0]]).T], axis=1)
        df_median_group_1 = pd.concat([
            df_median_group_1, pd.DataFrame(dict(mean_group_1), index=[group[0]]).T], axis=1)
    ...
    with pd.ExcelWriter(os.path.join(save_folder_tmp, 'Stats.xlsx'),
        ...
        df_median_group_0.to_excel(writer, sheet_name='Median group 0', index=True)
        df_median_group_1.to_excel(writer, sheet_name='Median group 1', index=True)
...
def lookup_oddsratio(odds_ratio):
    if odds_ratio < 0.34:
        ...
        return 'greater, moderate effect'
    elif odds_ratio > 2.9:
...
def perform_test_nominal_category(df, nominal_cols, encoded__labels, save_folder):
    ...
    for group in names:
        ...
        for nominal_attr in nominal_cols_encoded:
            ...
            mean_group_0.append((nominal_attr, df_melted_tmp.loc[
        ...
        df_chisquare = pd.concat([pd.DataFrame(d_swap, index=[group[0]]).T], axis=1)
    ...
    with pd.ExcelWriter(os.path.join(save_folder_tmp, 'Stats.xlsx'),
        ...
        df_effect_size.to_excel(writer, sheet_name="Cliff's Delta", index=True)
        df_effect_size_categorised.to_excel(writer, sheet_name="Cliff's Delta categorised", index=True)
        df_mean_group_0.to_excel(writer, sheet_name='Mode group 0', index=True)
...
def plot_clustered_summary(df, save_folder, key, pval_cut=0.05, figsize=(16, 12)):
    ...
    # df_tmp_[df_tmp_ > -np.log10(pval_cut)] = np.nan
    ...
    sns.heatmap(df_tmp_clustered, vmin=0, xticklabels=True, yticklabels=True, ax=axmatrix,
                cmap=cmap, center=-np.log10(pval_cut), cbar=False, linewidths=1.3)
    ...
    cbar = plt.colorbar(mappable=axmatrix.collections[0], cax=axcolor,
                        ticks=np.arange(0, np.amax(df_tmp_clustered), -np.log10(pval_cut)))
    cbar.ax.get_yaxis().labelpad = 15
    cbar.ax.set_ylabel('-log10(p-values)', rotation=90, fontsize=12)
...
def plot_clustered_comparisons_summary(df, df_effect_size, save_folder, key, xlabels, pval_cut=0.05, figsize=(16, 12)):
    df = df.dropna(axis='index', how='all').astype(float)
    ...
    # w=0.03
    ax2 = fig.add_axes([0.16, 0.1, 0.2, 0.72])  # (left, bottom, width, height) fig.add_axes([0.095, 0.1, 0.1, 0.7])
    ...
    # l = 0.2
    # (np.amax(df_pattern_2a) + np.amin(df_pattern_2a))/ 2
    axmatrix = fig.add_axes([0.37, 0.1, 0.28, 0.72])  # (left, bottom, width, height)
    ...
    # l = 0.5
    axmatrix = fig.add_axes([0.67, 0.1, 0.14, 0.72])  # (left, bottom, width, height)
    ...
    # l = 0.66
    axmatrix = fig.add_axes([0.83, 0.1, 0.19, 0.72])  # (left, bottom, width, height)
    ...
    # Add colorbar
    # l = 0.2
    axcolor = fig.add_axes([0.37, 0.03, 0.65, 0.02])  # (left, bottom, width, height) fig.add_axes([0.9, 0.1, 0.03, 0.72])
    if pval_cut == 0.05:
        cbar = plt.colorbar(
            mappable=axmatrix.collections[0], cax=axcolor, orientation="horizontal",
            ticks=[np.amin(df_tmp_clustered)] + [-3] + [0] + [4, 8, 12] + [np.amax(df_tmp_clustered)])
    else:
        cbar = plt.colorbar(
            mappable=axmatrix.collections[0], cax=axcolor, orientation="horizontal",
            ticks=[np.amin(df_tmp_clustered)] + [-12] + [
                np.log10(pval_cut)] + [0] + [-np.log10(pval_cut)] + [12] + [np.amax(df_tmp_clustered)])
    # cbar.ax.get_xaxis().labelpad = 15
    cbar.ax.set_xlabel('signed[-log10(padj. value)]', rotation=0, fontsize=12)

    # Add heatmap showing the colors of the clusters
    ax_colors = fig.add_axes([0.37, 0.825, 0.65, 0.02])  # (left, bottom, width, height)
    cmap_colors = LinearSegmentedColormap.from_list('', df_colors.iloc[0, :].values, len(df_colors.iloc[0, :].values))
    gradient = np.linspace(0, 1, len(df_colors.columns))
    gradient = np.vstack((gradient, gradient))
...
def plot_clustered_samples_summary(df, save_folder, key, figsize=(16, 10)):
    ...
    cbar.ax.get_yaxis().labelpad = 15
    cbar.ax.set_ylabel('-log10(padj. value)', rotation=90, fontsize=12)
    sns.despine(fig=fig)
...
def plot_comparisons_summary(df, df_effect_size, save_folder, key, xlabels, pval_cut=0.05, figsize=(16, 12)):
    ...
    # Add colorbar
    axcolor = fig.add_axes([0.2, 0.03, 0.65, 0.02])  # (left, bottom, width, height) fig.add_axes([0.9, 0.1, 0.03, 0.72])
    if pval_cut == 0.05:
        cbar = plt.colorbar(
            mappable=axmatrix.collections[0], cax=axcolor, orientation="horizontal",
...
def plot_selected_attributes(df, save_folder, key, xlabels, pval_cut=0.05, figsize=(16, 12)):
    ...
    cbar = plt.colorbar(mappable=axmatrix.collections[0], cax=axcolor,
                        ticks=[0, -np.log10(pval_cut)] + list(
    ...
    plt.savefig(os.path.join(save_folder, "Summary_selected_attr_Heatmap_{}_pval_{}.pdf".format(key, pval_cut)),
                bbox_inches='tight')
    plt.close()


def plot_selected_attributes_effect_size(df, df_effect_size, save_folder, key, xlabels, pval_cut=0.05, figsize=(16, 12)):
    df = df.dropna(axis='index', how='all').astype(float)
    df_effect_size = df_effect_size.dropna(axis='index', how='all').astype(float)
    df_copy = df.copy()
    df_copy = df_copy.dropna(axis='index', how='all')
    ...
    if pval_cut == 0.05:
        cbar = plt.colorbar(
            mappable=axmatrix.collections[0], cax=axcolor, orientation="horizontal",
            ticks=[np.amin(df_tmp_)] + [-4] + [0] + [6, 10, 14] + [np.amax(df_tmp_)])
    else:
        cbar = plt.colorbar(
            mappable=axmatrix.collections[0], cax=axcolor, orientation="horizontal",
            ticks=[np.amin(df_tmp_)] + [-10] + [
                np.log10(pval_cut)] + [0] + [-np.log10(pval_cut)] + [12] + [np.amax(df_tmp_)])
    # cbar.ax.get_xaxis().labelpad = 15
    cbar.ax.set_xlabel('-log10(p-values)', rotation=0, fontsize=12)
...
def main(input_dir, input_meta_dir, save_folder):
    ...
    # correct for FDR:
    # # per group
    # df_padj = pd.DataFrame(index=df_merged.index, columns=df_merged.columns)
    # for col in df_merged.columns:
    #     _, pvals_fdr_tmp, _, _ = multipletests(df_merged[col].tolist(), method='fdr_bh')
    #     df_padj[col] = pvals_fdr_tmp
    ...
    plot_summary(df=df_padj, save_folder=save_folder, key='merged', figsize=(12, 8))
    ...
    # plot_clustered_comparisons_summary(df=df_merged, df_effect_size=df_merged_effect_size, save_folder=save_folder,
    #                                    xlabels=list(adata.obs[obs_name].cat.categories),
    #                                    pval_cut=pval_cut, key='merged', figsize=figsize)
    plot_comparisons_summary(
        df=df_padj, df_effect_size=df_merged_effect_size, save_folder=save_folder,
    ...
    keep = ['Hist_ID_quant', 'Hist_Nr_Keratoses', 'Hist_Granu', 'Hist_Para_quant', 'Hist_Aka_quant',
            ...
            'Hty_photos_2.0']
    attr_names = [
        'Histology - Interface dermatitis (quantitative)', 'Histology - Dyskeratoses (quantitative)',
        ...
        'History - Leukocytoclastic vasculitis', 'Comorbidity - Mucosa involved', 'History - photosensitivity']
    rename_attr = dict(zip(keep, attr_names))

    # Select attributes - for thesis
    df_merged_subset = df_padj.loc[keep, :]
    df_merged_subset.to_excel(os.path.join(save_folder, 'Heatmap_enriched_clinical_attributes_Endotypes_Thesis.xlsx'))
    df_merged_effect_size_subset = df_merged_effect_size.loc[keep, :]
    df_merged_effect_size_subset.to_excel(os.path.join(
        save_folder, 'Heatmap_enriched_clinical_attributes_effect_size_Endotypes_Thesis.xlsx'))

    # rename attributes (rows)
    df_merged_subset.rename(index=rename_attr, inplace=True)
    df_merged_effect_size_subset.rename(index=rename_attr, inplace=True)

    # plot_clustered_summary(df=df_merged_subset, save_folder=save_folder, pval_cut=pval_cut, key='thesis', figsize=(18, 12))
    # plot_clustered_comparisons_summary(
    #     df=df_merged_subset, df_effect_size=df_merged_effect_size_subset, save_folder=save_folder,
    #     xlabels=list(adata.obs[obs_name].cat.categories), pval_cut=pval_cut, key='thesis', figsize=figsize)
    plot_comparisons_summary(
        df=df_merged_subset, df_effect_size=df_merged_effect_size_subset, save_folder=save_folder,
        xlabels=list(adata.obs[obs_name].cat.categories), pval_cut=pval_cut, key='thesis', figsize=figsize)

    # Main figure: keep only attributes:
    # Histology - Interface dermatitis (quantitative)
    # Histology - Dyskeratoses (quantitative)
    # Histology - Abnormal Str. granulosum
    # Histology - Parakeratosis (quantitative)
    # Histology - Akanthosis (quantitative)
    # Histology - Neutrophils (quantitative)
    # Histology - Dermal lymphocytes (quantitative)
    # Morphology - Pustules
    # History - Leukocytoclastic vasculitis
    # Comorbidity - Mucosa involved
    ...
    df_merged_subset = df_padj.loc[keep, :]
    # save data
    df_merged_subset.to_excel(os.path.join(save_folder, 'Heatmap_enriched_clinical_attributes_Endotypes.xlsx'))
    # rename attributes (rows)
    df_merged_subset.rename(index=rename_attr, inplace=True)

    # save data
    df_merged_subset.to_excel(os.path.join(save_folder, 'Heatmap_enriched_clinical_attributes_Endotypes_renamed.xlsx'))

    # heatmap plot without clustering of rows, because we do not cluster p-values ..
    plot_selected_attributes(df=df_merged_subset, save_folder=save_folder,
                             xlabels=list(adata.obs[obs_name].cat.categories),
                             pval_cut=pval_cut, key='', figsize=(6, 6))

    df_merged_effect_size_subset = df_merged_effect_size.loc[keep, :]
    df_merged_effect_size_subset.rename(index=rename_attr, inplace=True)

    plot_selected_attributes_effect_size(
        df=df_merged_subset, df_effect_size=df_merged_effect_size_subset, save_folder=save_folder,
        xlabels=list(adata.obs[obs_name].cat.categories), pval_cut=pval_cut,
        key='thesis_effect_size', figsize=(6, 6))


if __name__ == '__main__':
    general_dir = os.path.join(
        '/Volumes', 'CH__data', 'Projects', 'Eyerich_AG_projects', 'BRAIN__Natalie_Garzorz-Stark_Peter_Seiringer')
    output_dir = os.path.join(general_dir, 'analysis', 'Molecular_subtypes', 'output', 'Figure_3D', str(date.today()))
