...
def export_legend(legend, save_folder, ncol, filename="legend", expand=[-5, -5, 5, 5]):
    ...
    # fig.tight_layout()
    fig.savefig(os.path.join(save_folder, "Figure_2A_{}_ncol{}{}".format(
...
def main(save_folder):
    ...
    adata = adata[~adata.obs['Pattern'].isin(['2b', '4a', '4b', 'UD'])].copy()  # 282 samples
    ...
    fig.tight_layout()
    plt.savefig(os.path.join(save_folder, "Figure_2A_UMAP_{}.pdf".format(obs)))
    plt.close(fig=fig)
...
if __name__ == '__main__':
    output_dir = os.path.join('/Volumes', 'CH__data', 'Projects', 'Eyerich_AG_projects',
                              ...
                              'output', 'Figure_2A_UMAP_majority_pattern', str(date.today()))
